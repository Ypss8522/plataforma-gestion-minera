
# Backend — Sistema de Acreditación Minera
# Documentación Técnica Maestra 
## Sistema de Gestión de Acreditación, Operaciones y Gerencia — Rubro Minero

Esqueleto basado en la Documentación Técnica Maestra v1.0.

**Stack:** NestJS (TypeScript) + Prisma + PostgreSQL + Redis (BullMQ) + JWT/RBAC + RLS + Cloudflare R2.

## Estructura de módulos (calcada a los actores del dominio)

```
src/
├── auth/              # Login, JWT strategy
├── acreditacion/      # RRHH — documentos, cursos, cálculo de semáforo (RN-01)
├── operaciones/       # Búsqueda por competencia, frentes de trabajo (RN-03, RN-04)
├── gerencia/          # Reportes KPI (RN-02, RN-07)
├── mobile/            # App del Trabajador — aislamiento absoluto (RN-08)
├── common/
│   ├── decorators/    # @Roles, @CurrentUser, @Public
│   ├── guards/        # JwtAuthGuard, RolesGuard, MobileSelfAccessGuard
│   ├── storage/        # StorageService (Cloudflare R2 vía SDK de S3)
│   └── utils/          # semaforo.util.ts (RN-01)
└── prisma/            # PrismaService con soporte de contexto RLS
```

## Cómo levantar el proyecto

```bash
# 1. Instalar dependencias
npm install

# 2. Copiar variables de entorno
cp .env.example .env
# editar .env con tu DATABASE_URL real, JWT_SECRET fuerte, y credenciales de R2

# 3. Levantar Postgres y Redis (docker-compose incluido)
docker compose up -d

# 4. Generar cliente Prisma y correr migraciones
npm run prisma:generate
npm run prisma:migrate

# 5. Aplicar las políticas RLS (Prisma no las gestiona nativamente)
psql "$DATABASE_URL" -f prisma/rls-policies.sql

# 6. Cargar datos semilla (mineras Antapacay/Las Bambas, tags, matriz de requisitos)
npm run prisma:seed

# 7. Levantar en modo desarrollo (dos terminales)
# Terminal 1 — compila en watch:
npm run dev:build
# Terminal 2 — ejecuta y reinicia al detectar cambios:
npm run dev:run
```

> **Nota histórica:** `nest start --watch` presentó fallas de timing
> intermitentes en este proyecto (`Cannot find module dist/main`), causadas
> por caché incremental de TypeScript corrupta (`*.tsbuildinfo`) y una
> detección automática de NestJS en `nodemon` 3.1.x que sobreescribía el
> comando a ejecutar. Por eso el flujo usa dos procesos separados y
> explícitos. Si `dist/main.js` no aparece pese a "0 errors" en la Terminal 1,
> sospecha primero de `.tsbuildinfo` corrupto:
> `find . -name "*.tsbuildinfo" -not -path "./node_modules/*" -delete`

API disponible en `http://localhost:3000/api/v1`.

## Capas de seguridad implementadas en este esqueleto

1. **Helmet** — headers HTTP seguros (main.ts).
2. **ValidationPipe global** con `whitelist` + `forbidNonWhitelisted` — previene mass assignment.
3. **JwtAuthGuard global** — todo endpoint requiere JWT válido salvo `@Public()`.
4. **RolesGuard** — RBAC declarativo vía `@Roles(...)`.
5. **MobileSelfAccessGuard** — bloquea cualquier intento de un TRABAJADOR de leer datos de otro trabajador.
6. **Row-Level Security en Postgres** (`prisma/rls-policies.sql`) — defensa en profundidad.
7. **Rate limiting** — global (100 req/min) y reforzado en `/auth/login` (5 intentos/min).
8. **bcrypt** para hash de contraseñas.
9. **Storage (Cloudflare R2)** — URLs firmadas de 5 min, validación de magic bytes (no solo mimeType declarado), nombres de archivo con UUID (no predecibles), fail-fast si faltan variables de entorno.

## CI/CD

Ver [`docs/CI_CD.md`](./docs/CI_CD.md) para la guía completa de configuración de GitHub Actions (CI) y Render (CD).

## Pendiente para completar Sprint 1

- [ ] Worker BullMQ: job diario de recálculo de `trabajador_estado_contexto` y disparo de notificaciones (push/email).
- [ ] Endpoints de `matriz-cumplimiento` (bulk) y `candidato-cargo` (sección 4 del doc maestro).
- [ ] Endpoint de lectura/visualización de documentos ya subidos (ver patrón de seguridad en `docs/CI_CD.md` — el fileKey debe salir SIEMPRE de una consulta a `documento` filtrada por trabajador, nunca de un parámetro de la request).
- [ ] Tests unitarios de los guards de seguridad (crítico: `MobileSelfAccessGuard` y RN-04).
